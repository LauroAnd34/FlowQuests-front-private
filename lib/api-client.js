...
async function loginUser(credentials) {
  return withFallback(
    ...
    () => {
      ...
      if (userByEmail && userByEmail.senha === credentials.senha && userByEmail.statusConta !== 'ATIVO') {
      ...
      const user = mockState.users.find(
        (item) => item.email.toLowerCase() === normalizedEmail && item.senha === credentials.senha
      );
  ...
  return withFallback(
    ...
    () => {
      ...
      if (senha.length < 6) {
        ...
        error.response = { status: 400, data: { message: 'Senha deve ter ao menos 6 caracteres.' } };
        throw error;
      }
